...
type DashboardSnapshot = z.infer<typeof publicContract.dashboardSchema>;
type PredictionRow = z.infer<typeof publicContract.predictionSchema>;
type MatchRow = z.infer<typeof publicContract.matchSummarySchema>;

type DashboardExperienceProps = {
  initialDashboard: DashboardSnapshot;
  initialPredictions?: PredictionRow[];
  initialMatches?: MatchRow[];
  initialDashboardUnavailable?: boolean;
};
...
export function DashboardExperience({
  ...
  initialPredictions = [],
  initialMatches = [],
  initialDashboardUnavailable = false
  ...
  const predictionsQuery = useQuery({
    queryKey: ["public-dashboard-predictions"],
    queryFn: async () => {
      const response = await fetchWithSchema(
        "/api/v1/predictions?status=scheduled,live,finished&take=220",
  ...
  const matchesQuery = useQuery({
    queryKey: ["public-dashboard-matches"],
    queryFn: async () => {
      const response = await fetchWithSchema("/api/v1/matches?take=80", publicContract.matchesResponseSchema, {
        cache: "no-store"
      });
      return response.data;
    },
  ...
  const predictions = predictionsQuery.data ?? initialPredictions;
  const matches = matchesQuery.data ?? initialMatches;
  ...
  const recentActivities = useMemo<RecentActivityItem[]>(() => {
    ...
    return rows
      ...
      .slice(0, 6);
  }, [dashboard.generatedAt, matches, predictions]);

  const shouldShowWarning =
    (initialDashboardUnavailable || dashboardQuery.isError) &&
    dashboard.matchCount === 0 &&
    dashboard.predictionCount === 0 &&
