...
export class ApiFootballConnector {
  private readonly defaultBaseUrl = "https://v3.football.api-sports.io";
  ...
  async fetchFixtures(apiKey: string, date: string, baseUrl?: string) {
    const response = await this.request(
      `${this.resolveBaseUrl(baseUrl)}/fixtures?date=${date}`,
      {
        headers: this.headers(apiKey)
      },
    ...
    if (!response.ok) {
      throw new Error(`api_football fixtures failed: ${response.status}`);
    }
