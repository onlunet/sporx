...
@Injectable()
export class ApiBasketballConnector {
  private readonly defaultBaseUrl = "https://v1.basketball.api-sports.io";
  ...
  async ping(apiKey: string, baseUrl?: string) {
    const response = await this.request(
      `${this.resolveBaseUrl(baseUrl)}/status`,
      {
  ...
  async fetchGames(apiKey: string, date: string, baseUrl?: string) {
    const response = await this.request(
      `${this.resolveBaseUrl(baseUrl)}/games?date=${date}`,
      {
        headers: this.headers(apiKey)
      },
      "api_basketball games"
    );

    if (!response.ok) {
      throw new Error(`api_basketball games failed: ${response.status}`);
    }

    return (await response.json()) as { response?: Array<Record<string, unknown>> };
  }
}
